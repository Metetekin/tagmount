...
if (isset($_POST['video_id'])) {
  ...
  $query = $db->prepare("SELECT p.*,v.VIDEO_KEY FROM pointer p left join video v ON v.ID=p.VIDEO_ID WHERE  v.VIDEO_KEY=:video_id AND p.ACTIVE=1 order by p.ID "); //Tüm verileri getir
